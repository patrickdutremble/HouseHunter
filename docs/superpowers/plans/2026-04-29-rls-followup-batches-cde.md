...
> **POST-MORTEM CORRECTION (added after the original plan shipped):** The original Task 4 plan applied `requireEnv` to `client.ts` and broke production. Reason: Next.js only inlines `NEXT_PUBLIC_*` env vars into the browser bundle when you write *direct* property access (`process.env.NEXT_PUBLIC_X`). `requireEnv` does a *dynamic* lookup (`process.env[name]`), which Next.js cannot inline. In the browser, the lookup returned `undefined` and `requireEnv` threw on every page load. The fix shipped in commit `000c0ad` keeps direct property access in `client.ts` and validates non-empty at module load — see the current file for the canonical pattern. **Only apply `requireEnv` to code paths that run in Node.js or Edge Runtime, never to browser bundles.**

`middleware.ts` (the helper, NOT the project-root file) runs in the Edge Runtime per request. `requireEnv` is safe there because the Edge Runtime DOES support dynamic `process.env[name]` lookups — env vars are populated at process start, not inlined at build time. `requireEnv` is called inside `updateSession()`, so it throws on the first request if env vars are missing. Note: `import 'server-only'` is **not** appropriate for the Edge Runtime because it doesn't honour the `react-server` export condition; the helper's protection comes from being imported only by `proxy.ts`.
